...
CREATE TABLE tracklets (
  ...
  semantic_vector vector(1152),              -- SigLIP so400m-patch16-224 (search)  Phase 1  [LOCKED]
...
- *SigLIP semantic_vector*: **LOCKED in Phase 0** to `google/siglip2-so400m-patch16-224` (**1152-dim**), the schema hardcodes this dim. We picked the larger so400m over `base`/768 up front because search is the headline demo and so400m retrieves noticeably better; it fits 6 GB VRAM (only model resident during the SigLIP pass) and the +50% index size is trivial at a few-thousand rows. Preprocess per the model's processor. This same model's **text** encoder embeds the user's query at search time. (Never switch models later — it would force re-embedding every vector.)
...
- **Phase 5 — People & plates (additive).** A **person-attribute** pass (outfit region colors → `person_attrs`) and **plate OCR** (`plate_text`) as new passes over existing crops — fill null columns, no re-detection, no re-embedding. Free-text "man in red shirt" already works because SigLIP handles people out of the box.
